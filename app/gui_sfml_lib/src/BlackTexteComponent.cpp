#include "../include/BlackTexteComponent.hpp"
#include <iostream>
#include <filesystem>
#include <sstream>
#include <algorithm>
...
namespace Plazza::Components {
    ...
    BlackTexteComponent::BlackTexteComponent(const std::string& content)
    ...
    void BlackTexteComponent::setText(const std::string& content) {
        _rawText = content;
        wrapText();
    }

    void BlackTexteComponent::setPosition(float startX, float endX, float startY, float endY) {
        _startX = startX;
        _endX = endX;
        _startY = startY;
        _endY = endY;
    ...
    int BlackTexteComponent::countLines(const std::string& str) {
        return std::count(str.begin(), str.end(), '\n') + 1;
    }

    void BlackTexteComponent::wrapText() {
        std::string wrapped;
    ...
    void BlackTexteComponent::draw(sf::RenderWindow& window) {
        window.draw(_text);
    }

    std::string BlackTexteComponent::getText() const {
        return _rawText;
    }
